...
function TableHeader({className}: HeaderProps) {
	const headers: string[] = ["name", "email", "address", "dateOfBirth", "Action"];
	return (
		<>
		...
		<>
			<tr className={className}>
				...
				<td className={"text-center p-1"}>
					<button className={"border pl-3 pr-3 rounded-lg mr-1 border-blue-800 transition-all duration-150 cursor-pointer hover:bg-blue-800 hover:text-white"}
									onClick={() => handleEditPatient(patient)}>Edit</button>
					<button className={"border pl-3 pr-3 rounded-lg ml-1 border-red-700 transition-all duration-150 cursor-pointer hover:bg-red-700 hover:text-white"}
		...
		<>
			<div className={"m-auto w-4/5 max-w-5xl p-4 border-2 border-gray-400 rounded-2xl " + className}>
				<table className={"w-full"}>
					...
					<tbody>
						{patients.map((p) => (
							<TableRow className={"border-b-1 border-gray-300 hover:bg-blue-50"} key={p.id} patient={p}
